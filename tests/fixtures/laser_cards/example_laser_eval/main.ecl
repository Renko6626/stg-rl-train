// 激光夹具（评测用，参数与 example_laser 略异）：无敌 boss，每 60 帧放一组三种形态的激光（预警扫射 · 自机狙 · 飞出去的短棒），原点左右不对称。
const TIME_LIMIT: int = 1800;

async sub laser_pattern() {
    wait(20);
    loop {
        var sweep: int = laser(4, 60.0fx, 110.0fx, 120deg, 500.0fx, 24.0fx, 20, 60, 10);
