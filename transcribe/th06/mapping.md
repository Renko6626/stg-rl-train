...
- **取整随机会取到端点**：`set_float_rand_bound($Fx, r)` 翻成 `r / 256 * rand(256)` 时，`rand(256) == 0` 有 1/256 的概率让结果**正好是 0**；
  原作是连续随机，几乎取不到 0。结果若要喂给「非零才钳位」的逻辑（如 §4.2 的 `s1 < 0.3 → 0.3`，`s1 == 0` 豁免），
  会绕过钳位得到静止弹。这种情况写 `r / 256 * (rand(256) + 1)` 或先钳后用，并写进 report（2026-09-26 s7_b12 便笺）。
