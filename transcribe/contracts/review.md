...
5. **数值锚点（硬性，至少 3 条）**：从原文**手算**某帧的期望，用 harness 实测对照。
   **期望值只能从 `source.txt` 的原作参数推出来**（颗数、层数、速度、角度、周期、首发帧），**禁止照抄卡里的注释或 report 当期望**——
   卡写错时注释往往跟着错，照抄就等于拿错的卡验证错的卡（2026-09-25 opus 抽检：s7_b17 把颜色参数当颗数，s4_b13/b14 把 36 帧周期当期望，dsh 都判了 pass）。
   时序类期望按对照表 §2.4（条件跳转设绝对时间）与 §4.3（自动射击首发 = S + n − 1）重新算，不要沿用旧口径「晚 1 帧可接受」。优先用发弹锚点：
