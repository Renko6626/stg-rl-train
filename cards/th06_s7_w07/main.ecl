// th06_s7_w07 —— 东方红魔乡 Stage 7(Extra) 道中第 7 波
// 原文：ecldata7 timeline 帧 3040（Sub2 单只，x=+64）；Extra 档
...
async sub sub2() {
    ...
    // bullet_circle(3, 6, 13, 1, %F1, 1.0f, %F0, 0.0f, 513)
    // 弹型 3 → BALL，色 6 原样；13 颗整周、单层速度 = f1；flags 0x1 → BURST
    sh_reset(0);
    sh_sprite(0, BALL, 6);
    sh_aim(0, 0);
    sh_ring(0, 1);
...
// 导演：120 帧开场缓冲后按 timeline 帧 3040 出唯一一只 Sub2（x=256−192=+64）
async sub wave() {
    wait(120);
    _ = spawn_enemy(64.0fx, -48.0fx, 1, 0, 0, 0, sub2);   // x=+64, y=-48
    loop { wait(1); }
}
