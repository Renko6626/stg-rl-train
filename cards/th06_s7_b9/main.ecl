// th06_s7_b9 —— 东方红魔乡 Stage 7(Extra) boss 非符 5
// 原文：ecldata7 Sub61（:1680-1724），全档 !ENHL；timer/life_callback_threshold(1800/1400) → Sub62。
...
// bullet_circle(2, 6, 32, 2, 2.5f, 1.0f, %F0, 0.0f, 513) + shoot_interval(30)
// 单次配置：32 颗 × 2 层，速度 2.5/1.75，整周环、基准角随机；之后每 30 帧自动射击。
async sub autoshoot(interval: int, until: int) {
    ...
    sh_ring(0, 1);
    sh_count(0, 32, 2);
    sh_speed(0, 2.5fx, -0.75fx);            // (s2 − s1) / c2 = (1.0 − 2.5) / 2
    sh_angle(0, a0, 0deg);
    sh_xform(0, BURST);
    var k: int = interval - 1;              // §4.3：首发 k = n − 1（相对设定帧 S）
    if k >= until { return; }
    if k > 0 { wait(k - 1); }               // 子任务 S+1 才首跑，wait(k−1) 正好落在 S+k
    loop {
        sh_fire(0);
...
async sub pattern() {
    ...
    spawn autoshoot(30, 1710);              // shoot_interval(30)；S = T90，until = 1800 − 90；k = 29…1709 共 57 轮
    var i7: int = 0;                        // set_int($I7, 0)
    loop {
        wander(2.5fx, 90);                  // Sub61_532
        wait(100);                          // +100 //190
        i7 = i7 + 1;                        // math_inc($I7)
        wait(10);                           // +10 //200 → jump(90, Sub61_532)
    }
}
