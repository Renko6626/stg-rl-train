// th06_s7_b7 —— 东方红魔乡 Stage 7(Extra) boss 非符 4
// 原文：ecldata7 Sub55（:1511-1555），全档 !ENHL；timer_callback_threshold(1800)。
...
// bullet_circle(2, 6, 32, 2, 2.0f, 1.0f, %F0, 0.0f, 513) + shoot_interval(40)
// 单次配置：32 颗 × 2 层，速度 2.0/1.5，整周环、基准角随机；之后每 40 帧自动射击。
async sub autoshoot(interval: int, until: int) {
    ...
    sh_ring(0, 1);
    sh_count(0, 32, 2);
    sh_speed(0, 2.0fx, -0.5fx);             // (s2 − s1) / c2 = (1.0 − 2.0) / 2
    sh_angle(0, a0, 0deg);
    sh_xform(0, BURST);
    var k: int = interval - 1;              // 首发 k = n − 1（§4.3），子任务次帧起跑故 wait(k − 1)
    if k >= until { return; }
    if k > 0 { wait(k - 1); }
    loop {
        sh_fire(0);
...
async sub pattern() {
    ...
    spawn autoshoot(40, 1710);              // shoot_interval(40)；until = 1800 − 90（本任务起点）
    var i7: int = 0;                        // set_int($I7, 0)
    loop {
        wander(2.5fx, 90);                  // Sub55_532
        wait(100);                          // +100 //190
        i7 = i7 + 1;                        // math_inc($I7)
        wait(10);                           // +10 //200 → jump(90, Sub55_532)
    }
}
