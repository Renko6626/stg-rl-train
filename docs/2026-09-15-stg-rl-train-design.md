...
> 状态：**第一刀已落地**（训练仓 `Renko6626/stg-rl-train`；实施计划 `docs/2026-09-15-stg-rl-train-plan.md`）。GPU 验收（§8）待首次上 Vast.ai 回填。
...
> 本文已从 stg-engine 迁入训练仓 `docs/`（来源 commit 见首行），以本仓这份为准。
...
| ⑤ | 遵从 reward | 势函数距离塑形 + 半径 R 内停留奖励；终局步不计塑形，刷新步用旧指令点照算（§3.5、§13 #1） |
...
| `follow_shaping` | `γ·Φ(s') − Φ(s)`，`Φ = −d / 448`，d = 自机到指令点距离；**终局步（任意 done）记 0**；刷新步用本步动作所瞄的旧指令点照算（§13 #1） | 1.0 |
| `hold` | `d < R` 的帧 +1 | 0.01 |
...
- `reward`：γ = 1 时两次刷新之间的塑形总和 = Φ(段末) − Φ(段首)，一局内总和 = 各刷新段 Φ 差之和（刷新步用旧指令点照算，§13 #1）；终局步记 0；三种 done 码；配置校验（`death` 过小时报错）。
...
写计划（`docs/2026-09-15-stg-rl-train-plan.md` §Rulings）时对本文的裁定：

| # | 偏差 | 理由 |
|---|---|---|
| 1 | 刷新步的遵从塑形不置 0，用旧指令点精确计算；只保留「终局步记 0」 | reward 在刷新之前算、两个距离都对 `prev.target_xy`，本就没有目标突变的跳变；§3.5 表格、§1 ⑤ 与 §8 单测描述已同步改写 |
