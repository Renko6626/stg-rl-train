...
先复用站点缓存的 CUDA 12.4 镜像，运行 `bash magnus/smoke.sh`。它在 Job 中安装 TensorDict 0.6.2 和少量运行依赖，再执行 `gpucheck`、两次 PPO 更新和结果回传。它使用 `PYTHONPATH=src`，暂时绕开主仓 Python 3.12 / PyTorch 2.14 的安装约束；通过只能证明 CUDA 12.4 方案可行，**不能**视为正式训练环境已经锁定。
...
1. 让 `gpucheck` 和两次 PPO 更新在 Magnus 上完成，核对结果包中的环境记录、评测和 checkpoint；再决定 TensorDict 和 PyTorch 的确切配对。
2. 给 Magnus 路径做独立锁定，不改变 Vast.ai 当前的 `uv.lock`；运行时必须记录实际的 Python、PyTorch、CUDA、TensorDict 与 wheel SHA。
3. 确认站点可用的持久存储，将 `--runs-dir` 指向持久路径。Magnus 会清理 Job 工作区，File Custody 只有短期有效，不能作为正式续训的唯一存储。
