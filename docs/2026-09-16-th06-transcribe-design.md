...
2. **`th06_s3_w12` 弹数超限 → 只收 E/N**：忠实转写后 H / L 弹峰值 1030 / 1630 > 卡池上限 1024。worker 查到根因：
   **TH06 弹池只有 640 发**（`BulletManager.hpp:125`，池满 `SpawnBulletPattern` 整批中止），原作画面被池上限压住；
   我方弹池 8192，脚本里的弹全部实体化。任何压到 1024 以下的改法都要改颗数 / 批数 / 轮数（审核口径 critical）。
   处置：卡体不改，`meta.toml` `ranks` 收窄为 `[0, 1]`，notes 写明原因；dsh 复审 pass。
   **第 4–7 关弹更密，预计会更常撞上**；批量量大时再考虑「模拟 640 池上限」这类引擎侧或卡池口径的统一方案，不逐张抽稀。
...
训练侧保留每个标签至少一张；`th06_s1_b4` 与 `probe/splits.toml` 相同便于和探测 run 对照。训练侧 rank 2 起点 32 个
（另有 4 张只含 E/N 档的卡不进 rank 2 训练：3 张 TH06 按难度分的符卡 + s3_w12）。
