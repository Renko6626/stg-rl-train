...
**主会话分诊与修正**：
1. **对照表 §4.3 停火边界（已更正，见 §13.5）**：批量时依第 3 关 w05 的 dsh 审核把写法 A 的守卫从 `>` 改成 `>=`，
   并让 s2_w04、s3_w01、s3_w06 返工。**这次改动是错的**：当时没考虑原作设定帧当帧就 tick，§13.5 已核实并改回 `>`。
...
### 13.5 抽检 th06_s3_mb1（2026-09-17，主会话对 decomp 逐条核）

**一致**：坐标换算（x−192）与移动边界；`bullet_random_speed` / `bullet_circle(_aimed)` 的角度与速度公式、L 档两层速度与 2° 层偏移；
`jump_dec`（先减后判 > 0）次数、`call`/`ret` 恢复调用方时间（攻击起点 130 帧）；`move_rand_in_bounds` 反射阈值、`move_time_accelerate`
位移 `speed·t/2` + QuadIn；自动射击出弹口沿用粘滞的 `shootOffset`；rank 修正（用户定为不模拟动态 rank，按 0 处理）。

**更正**：
1. **自动射击帧对齐与停火守卫**：`RunEcl` 在设定帧当帧就走开火计时，原作首发在第 `n−1` 帧；我方写法 A 统一晚 1 帧、次数一致，
   停火守卫应为 `>`。§13.4 第 1 条改成 `>=` 是错的，对照表已改回。已按 `>=` 入库的卡（s2_w02、s2_w03、s2_w04、s2_w05、s3_w01、
   s3_w04、s3_w05、s3_w06 中用到该守卫者）只在「开火帧恰好等于 until」时少打一轮，概率约 1/n，**不回改**，下一批按对照表写 `>`。
2. mb1 的 H/L 自动射击（`t == first + 30` 且 `t ≤ 50`）按上式正确，**不是**多打一发。

**有意不模拟（对照表 §4 已写明数字）**：出生特效 flags 2/4/8——原作 10/16/32 帧降速（×1/2、×1/2.5、×1/3）且无判定，
我方全速且有判定，弹位置领先 `5v/9.6v/21.3v` 像素。训练目标是泛化躲弹，不改引擎；部署侧须处理（stg-engine follow-ups D23#9）。

**小近似**：boss 越界时我方先夹紧终点再直线移动，原作逐帧夹紧（终点相同、中途路径略异）；H/L 出弹口偏移经全局槽取整到 1px。

**元数据**：s3_mb1、s3_b8 把 Stage 3 boss 写成「爱丽丝」（被 BGM 名《上海爱丽丝》带偏），已改为红美铃。
